...
#tf.train_dataset = tf.constant(train_dataset)
#tf.train_labels = tf.constant(train_labels)
...
with tf.Session() as sess:
  ...
  for i in range(2500):
    ...
    pred = y_conv.eval({x: train_dataset})
    
#%%
with tf.Session() as sess:
    test_pred = sess.run(tf.argmax(y_conv, 1), feed_dict={x: test_dataset})
